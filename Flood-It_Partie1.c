...
int main(int argc,char**argv){
    ...
    temps_initial = clock ();
    creer_graphe_zone(M,dim,nbcl);
    if (exo==0){
         printf("%d essais avec séquence rec. Temps mis =  ", sequence_aleatoire_rec(M, G, dim, nbcl, aff)); 
    }
    if (exo==1){
         printf("%d essais avec séquence imp. Temps mis = ", sequence_aleatoire_imp(M, G, dim, nbcl, aff)); 
    }
    if (exo==2){
        S_Zsg* Zsg = (S_Zsg *)malloc(sizeof(S_Zsg));
        printf("%d essais avec sequence rapide. Temps mis = ",strequence_aleatoire_rapide(M,G,dim,nbcl,Zsg,aff));
    }
